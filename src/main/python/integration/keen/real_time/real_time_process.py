...
class RealTimeProcess(Init):
    ...
    def _process_new_file(self, _unprocessed_files):
        ...
        try:
            ...
            proc = Popen(['hdfs', 'dfs', '-text', _unprocessed_file], stdout=PIPE)
            _output = proc.communicate()

            logging.info("process_new_file, number of lines in output = {}, output={}"
                         .format(len(_output), _output[0][:40]))
