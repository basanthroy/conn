...
class FileListRetrieverBidOverlap(FileListRetriever):
    ...
    def _construct_full_path_name(self, dt, hr):
        return self._entity_base_dir_template.format(dt)

    def construct_full_file_name(self, dt, hr, filename):
        return self._entity_filepath_template.format(dt, filename)

    def __init__(self, entity_base_dir_template, entity_filepath_template):
        super(FileListRetrieverBidOverlap, self)\
